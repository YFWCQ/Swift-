...
#import "FYBaseScroButtonsView.h"
#import "OCHeader.h"


@interface FYBaseScroButtonsView ()
...
@implementation FYBaseScroButtonsView
{
    //水平的X 坐标
    CGFloat _horX;
    // button 水平方向 的间距
...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        if (self.scroStyle == kFYScroIndicateLine)
        {
            _scroIndicView = [[UIView alloc] initWithFrame:CGRectMake(10, self.frame.size.height - 1, 5, 1)];
            
            _scroIndicView.backgroundColor = self.scroStyleColor;

            
        }else if (self.scroStyle == kFYScroIndicateDot)
        {
...
{
    _titleNomalColor = RGB_FY(153., 153., 153.);
    _titleSelectColor = RGB_FY(51., 153., 51.);
    _titleFont = [UIFont systemFontOfSize:17.];
    self.scroStyle = kFYScroIndicateLine;
    self.scroStyleColor =RGB_FY(51., 153., 51.);
}
...
#warning 练习
    if (_categoryButtonsArray.count  > index +1)
    {
        ...
        if (self.scroStyle == kFYScroIndicateLine)
        {
            ...
            [self setFrameScroIndicViewSizeToFrame:CGRectMake(_scroIndicView.frame.origin.x, _scroIndicView.frame.origin.y, leftButton.frame.size.width + widthExtra, 1)];
        }
...
-(void)setSelectButton:(UIButton *)selectButton
{
    ...
    if (self.scroStyle == kFYScroIndicateDot)
    {
